...
export default function SmartSearch({ onNavigate, variant = "compact" }: { onNavigate?: () => void; variant?: "compact" | "hero" }) {
  ...
  const suggestions = useMemo<Suggestion[]>(() => {
    ...
    const fromDialysis: Suggestion[] = dialysisTopics
      .filter((topic) => [topic.title_ar, topic.title_en, topic.summary_ar, topic.summary_en].filter(Boolean).join(" ").toLowerCase().includes(lower))
      .slice(0, 3)
      .map((topic) => ({ label: bilingual(topic.title_ar, topic.title_en, lang).text, sub: t("search.dialysisHeading"), to: `/dialysis/${topic.id}`, icon: topic.icon || "💧" }));
    const fromNicu: Suggestion[] = nicuTopics
      .filter((topic) => [topic.title_ar, topic.title_en, topic.summary_ar, topic.summary_en].filter(Boolean).join(" ").toLowerCase().includes(lower))
      .slice(0, 3)
      .map((topic) => ({ label: bilingual(topic.title_ar, topic.title_en, lang).text, sub: t("search.nicuHeading"), to: `/nicu/${topic.id}`, icon: topic.icon || "👶" }));
    const fromPharmacology: Suggestion[] = pharmacologyTopics
      .filter((topic) => [topic.title_ar, topic.title_en, topic.summary_ar, topic.summary_en].filter(Boolean).join(" ").toLowerCase().includes(lower))
      .slice(0, 3)
      .map((topic) => ({ label: bilingual(topic.title_ar, topic.title_en, lang).text, sub: t("search.pharmacologyHeading"), to: `/pharmacology/${topic.id}`, icon: topic.icon || "🧪" }));
    const fromTerminology: Suggestion[] = terminologyTopics
      .filter((topic) => [topic.title_ar, topic.title_en, topic.summary_ar, topic.summary_en].filter(Boolean).join(" ").toLowerCase().includes(lower))
      .slice(0, 3)
      .map((topic) => ({ label: bilingual(topic.title_ar, topic.title_en, lang).text, sub: t("search.terminologyHeading"), to: `/terminology/${topic.id}`, icon: topic.icon || "🔤" }));
    const fromFirstAid: Suggestion[] = firstAidTopics
      .filter((topic) => [topic.title_ar, topic.title_en, topic.summary_ar, topic.summary_en].filter(Boolean).join(" ").toLowerCase().includes(lower))
      .slice(0, 3)
      .map((topic) => ({ label: bilingual(topic.title_ar, topic.title_en, lang).text, sub: t("search.firstAidHeading"), to: `/first-aid/${topic.id}`, icon: topic.icon || "🩹" }));
    const fromERReception: Suggestion[] = erReceptionTopics
      .filter((topic) => [topic.title_ar, topic.title_en, topic.summary_ar, topic.summary_en].filter(Boolean).join(" ").toLowerCase().includes(lower))
      .slice(0, 3)
      .map((topic) => ({ label: bilingual(topic.title_ar, topic.title_en, lang).text, sub: t("search.erReceptionHeading"), to: `/er-reception/${topic.id}`, icon: topic.icon || "🚨" }));
    const fromPediatrics: Suggestion[] = pediatricTopics
      .filter((topic) => [topic.title_ar, topic.title_en, topic.summary_ar, topic.summary_en].filter(Boolean).join(" ").toLowerCase().includes(lower))
      .slice(0, 3)
      .map((topic) => ({ label: bilingual(topic.title_ar, topic.title_en, lang).text, sub: t("search.pediatricsHeading"), to: `/pediatrics/${topic.id}`, icon: topic.icon || "🍼" }));
    const fromAnatomy: Suggestion[] = anatomyTopics
      .filter((topic) => [topic.title_ar, topic.title_en, topic.summary_ar, topic.summary_en].filter(Boolean).join(" ").toLowerCase().includes(lower))
      .slice(0, 3)
      .map((topic) => ({ label: bilingual(topic.title_ar, topic.title_en, lang).text, sub: t("search.anatomyHeading"), to: `/anatomy/${topic.id}`, icon: topic.icon || "🦴" }));
    const fromFundamentals: Suggestion[] = fundamentalsTopics
      .filter((topic) => [topic.title_ar, topic.title_en, topic.summary_ar, topic.summary_en].filter(Boolean).join(" ").toLowerCase().includes(lower))
      .slice(0, 3)
    ...
    const fromObgyn: Suggestion[] = obgynTopics
      .filter((topic) => [topic.title_ar, topic.title_en, topic.summary_ar, topic.summary_en].filter(Boolean).join(" ").toLowerCase().includes(lower))
      .slice(0, 3)
      .map((topic) => ({ label: bilingual(topic.title_ar, topic.title_en, lang).text, sub: t("search.obgynHeading"), to: `/obgyn/${topic.id}`, icon: topic.icon || "🤰" }));
    ...
    const results = [...fromDrugs, ...fromIcu, ...fromDialysis, ...fromNicu, ...fromPharmacology, ...fromTerminology, ...fromFirstAid, ...fromERReception, ...fromPediatrics, ...fromAnatomy, ...fromFundamentals, ...fromOperatingRoom, ...fromObgyn, ...fromProtocols, ...fromArticles].slice(0, 8);
    ...
    return results;
  }, [q, articles, drugs, icuTopics, dialysisTopics, nicuTopics, pharmacologyTopics, terminologyTopics, firstAidTopics, erReceptionTopics, pediatricTopics, anatomyTopics, fundamentalsTopics, operatingRoomTopics, obgynTopics, protocols, lang]);
