...
import { fetchTerminologyQuizQuestions, type TerminologyQuizQuestion } from "../lib/terminologyQuizApi";
...
export default function TerminologyQuizWidget() {
  const { lang, t } = useI18n();
  const [pool, setPool] = useState<TerminologyQuizQuestion[] | null>(null);
  const [index, setIndex] = useState<number | null>(null);
  const [chosen, setChosen] = useState<number | null>(null);

  useEffect(() => {
    let alive = true;
    fetchTerminologyQuizQuestions()
      .then((rows) => {
        if (!alive) return;
        setPool(rows);
  ...
  return (
    <div className="mb-6 rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-bold dark:text-white">{t("terminology.quizWidgetTitle")}</h3>
      </div>
      ...
      <div className="mt-3 space-y-1.5">
        {options.map((o, oi) => {
          ...
          let cls = "border-slate-200 dark:border-slate-700 hover:border-violet-300";
          if (chosen !== null) {
      ...
      {chosen !== null && explanation && (
        <div className="mt-2 rounded-lg bg-violet-50 p-2 text-xs text-violet-700 dark:bg-violet-500/10 dark:text-violet-300">💡 {explanation}</div>
      )}
      {chosen !== null && (
        <div className="mt-3 flex items-center justify-between">
          ...
          <button type="button" onClick={another} className="rounded-full bg-violet-500 px-3 py-1.5 text-xs font-bold text-white">
            {t("terminology.quizAnother")}
          </button>
        </div>
      )}
    </div>
