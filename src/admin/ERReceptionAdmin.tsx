...
import { fetchERReceptionTopics, upsertERReceptionTopic, deleteERReceptionTopic, type ERReceptionTopic, type ERReceptionTopicSection } from "../lib/erReceptionTopicsApi";
import MediaPicker from "./MediaPicker";

const EMPTY: ERReceptionTopic = {
  id: "",
  order_num: 0,
  title_ar: "",
  title_en: "",
...
export default function ERReceptionAdmin() {
  const [items, setItems] = useState<ERReceptionTopic[]>([]);
  const [editing, setEditing] = useState<ERReceptionTopic | null>(null);
  const [sections, setSections] = useState<ERReceptionTopicSection[]>([]);
  ...
  function load() {
    setLoading(true);
    fetchERReceptionTopics().then(setItems).catch(() => setItems([])).finally(() => setLoading(false));
  }

  useEffect(() => { load(); }, []);

  function startEdit(p: ERReceptionTopic | null) {
  ...
  function updateSection(idx: number, patch: Partial<ERReceptionTopicSection>) {
  ...
  async function remove(id: string) {
    ...
    await deleteERReceptionTopic(id);
    load();
  }
  ...
  return (
    <div dir="rtl" className="mx-auto max-w-4xl p-6">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-xl font-bold dark:text-white">🚨 إدارة قسم الاستقبال والطوارئ (Reception & Emergency) ({items.length})</h1>
        <button onClick={() => startEdit(null)} className="rounded-lg bg-red-700 px-4 py-2 text-white">+ إضافة موضوع</button>
      </div>
          ...
          {items.map((p) => (
            <div key={p.id} className="flex items-center justify-between rounded-xl border border-slate-100 bg-white p-3 dark:border-slate-800 dark:bg-slate-900">
              ...
              <div className="flex gap-2">
                <button onClick={() => startEdit(p)} className="text-sm text-red-700 dark:text-rose-400">تعديل</button>
      ...
      {editing && (
        <div className="space-y-4 rounded-xl border border-slate-100 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <input placeholder="التصنيف (مثال: الإنعاش القلبي الرئوي)" value={editing.category || ""}
            ...
            {sections.map((s, idx) => (
              <div key={s.id} className="mb-3 rounded-xl border-2 border-slate-100 p-3 dark:border-slate-800">
                ...
                <div className="mt-2 flex items-center gap-3">
                  ...
                  <button type="button" onClick={() => setPickerForSection(idx)} className="rounded-lg bg-rose-500 px-3 py-1.5 text-xs font-bold text-white">
          ...
          <div className="flex gap-3 border-t border-slate-200 pt-2 dark:border-slate-800">
            <button onClick={save} className="rounded-lg bg-red-700 px-5 py-2 text-white">حفظ</button>
