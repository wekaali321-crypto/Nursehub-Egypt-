...
import { fetchDialysisTopics, upsertDialysisTopic, deleteDialysisTopic, type DialysisTopic, type DialysisTopicSection } from "../lib/dialysisTopicsApi";
import MediaPicker from "./MediaPicker";

const EMPTY: DialysisTopic = {
  id: "",
  order_num: 0,
  title_ar: "",
  title_en: "",
...
export default function DialysisAdmin() {
  const [items, setItems] = useState<DialysisTopic[]>([]);
  const [editing, setEditing] = useState<DialysisTopic | null>(null);
  const [sections, setSections] = useState<DialysisTopicSection[]>([]);
  ...
  function load() {
    setLoading(true);
    fetchDialysisTopics().then(setItems).catch(() => setItems([])).finally(() => setLoading(false));
  }

  useEffect(() => { load(); }, []);

  function startEdit(p: DialysisTopic | null) {
  ...
  function updateSection(idx: number, patch: Partial<DialysisTopicSection>) {
  ...
  return (
    <div dir="rtl" className="mx-auto max-w-4xl p-6">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-xl font-bold dark:text-white">💧 إدارة قسم الغسيل الكلوي ({items.length})</h1>
      ...
      {editing && (
        <div className="space-y-4 rounded-xl border border-slate-100 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <input placeholder="التصنيف (مثال: الوصول الوعائي)" value={editing.category || ""}
