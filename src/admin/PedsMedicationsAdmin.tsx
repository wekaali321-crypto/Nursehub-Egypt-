...
import {
  ...
  upsertPedsMedication,
  deletePedsMedication,
  PEDS_CATEGORY_ORDER,
  type PedsMedication,
} from "../lib/pedsMedApi";

const EMPTY: PedsMedication = {
  id: "",
  order_num: 0,
  category: PEDS_CATEGORY_ORDER[0],
  subcategory: "",
  drug_name: "",
  concentration: "",
  drug_class: "",
...
export default function PedsMedicationsAdmin() {
  const [items, setItems] = useState<PedsMedication[]>([]);
  const [editing, setEditing] = useState<PedsMedication | null>(null);
  ...
  function startEdit(item: PedsMedication | null) {
  ...
  return (
    <div dir="rtl" className="max-w-4xl mx-auto p-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-bold dark:text-white">إدارة أدوية الأطفال ({items.length})</h1>
        <button onClick={() => startEdit(null)} className="bg-teal-600 text-white rounded-lg px-4 py-2">+ إضافة حالة</button>
      </div>
          ...
          {items.map((i) => (
            <div key={i.id} className="flex items-center justify-between bg-white border border-slate-100 rounded-xl p-3 dark:bg-slate-900 dark:border-slate-800">
              ...
              <div className="flex gap-2">
                <button onClick={() => startEdit(i)} className="text-teal-600 text-sm">تعديل</button>
      ...
      {editing && (
        <div className="bg-white border border-slate-100 rounded-xl p-5 space-y-3 dark:bg-slate-900 dark:border-slate-800">
          <div className="grid sm:grid-cols-2 gap-3">
            <input placeholder="id (فريد، إنجليزي، بدون مسافات — مثال peds25)" value={editing.id}
          ...
          <select value={editing.category} onChange={(e) => setEditing({ ...editing, category: e.target.value })}
            ...
            {PEDS_CATEGORY_ORDER.map((c) => <option key={c} value={c}>{c}</option>)}
          </select>
          ...
          <input placeholder="اسم الحالة أو الدواء" value={editing.drug_name}
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <input placeholder="التركيز" value={editing.concentration || ""}
              ...
              className={inp} />
            <input placeholder="التصنيف" value={editing.drug_class || ""}
          ...
          <div>
            ...
            <button onClick={() => setSteps([...steps, ""])} className="text-teal-600 text-sm">+ إضافة خطوة</button>
          </div>

          <div>
            <div className="font-semibold mb-1 text-sm dark:text-white">جدول الجرعات (مفتاح / قيمة)</div>
            {doseRows.map((r, i) => (
              <div key={i} className="flex gap-2 mb-2">
                <input placeholder="المفتاح مثل اسم الدواء" value={r.key} onChange={(e) => {
            ...
            <button onClick={() => setDoseRows([...doseRows, { key: "", value: "" }])} className="text-teal-600 text-sm">+ إضافة سطر</button>
          </div>

          <label className="flex items-center gap-2 dark:text-white">
            <input type="checkbox" checked={editing.is_high_alert}
              onChange={(e) => setEditing({ ...editing, is_high_alert: e.target.checked })} />
            حالة/دواء عالي الخطورة (High-Alert)
          </label>
