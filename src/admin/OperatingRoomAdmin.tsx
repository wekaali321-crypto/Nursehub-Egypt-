...
import { fetchOperatingRoomTopics, upsertOperatingRoomTopic, deleteOperatingRoomTopic, type OperatingRoomTopic, type OperatingRoomTopicSection } from "../lib/operatingRoomTopicsApi";
import MediaPicker from "./MediaPicker";

const EMPTY: OperatingRoomTopic = {
  id: "",
  order_num: 0,
  title_ar: "",
  title_en: "",
...
export default function OperatingRoomAdmin() {
  const [items, setItems] = useState<OperatingRoomTopic[]>([]);
  const [editing, setEditing] = useState<OperatingRoomTopic | null>(null);
  const [sections, setSections] = useState<OperatingRoomTopicSection[]>([]);
  ...
  function load() {
    setLoading(true);
    fetchOperatingRoomTopics().then(setItems).catch(() => setItems([])).finally(() => setLoading(false));
  }

  useEffect(() => { load(); }, []);

  function startEdit(p: OperatingRoomTopic | null) {
  ...
  function updateSection(idx: number, patch: Partial<OperatingRoomTopicSection>) {
  ...
  async function save() {
    ...
    await upsertOperatingRoomTopic({
      ...editing,
  ...
  async function remove(id: string) {
    ...
    await deleteOperatingRoomTopic(id);
    load();
  }
  ...
  return (
    <div dir="rtl" className="mx-auto max-w-4xl p-6">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-xl font-bold dark:text-white">🔪 إدارة قسم العمليات (Operating Room) ({items.length})</h1>
        <button onClick={() => startEdit(null)} className="rounded-lg bg-emerald-800 px-4 py-2 text-white">+ إضافة موضوع</button>
      </div>
          ...
          {items.map((p) => (
            <div key={p.id} className="flex items-center justify-between rounded-xl border border-slate-100 bg-white p-3 dark:border-slate-800 dark:bg-slate-900">
              ...
              <div className="flex gap-2">
                <button onClick={() => startEdit(p)} className="text-sm text-emerald-700 dark:text-blue-400">تعديل</button>
      ...
      {editing && (
        <div className="space-y-4 rounded-xl border border-slate-100 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <input placeholder="التصنيف (مثال: العلامات الحيوية)" value={editing.category || ""}
            ...
            {sections.map((s, idx) => (
              <div key={s.id} className="mb-3 rounded-xl border-2 border-slate-100 p-3 dark:border-slate-800">
                ...
                <div className="mt-2 flex items-center gap-3">
                  ...
                  <button type="button" onClick={() => setPickerForSection(idx)} className="rounded-lg bg-emerald-800 px-3 py-1.5 text-xs font-bold text-white">
          ...
          <div className="flex gap-3 border-t border-slate-200 pt-2 dark:border-slate-800">
            <button onClick={save} className="rounded-lg bg-emerald-800 px-5 py-2 text-white">حفظ</button>
