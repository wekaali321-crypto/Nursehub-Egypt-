...
import { fetchFirstAidTopics, upsertFirstAidTopic, deleteFirstAidTopic, type FirstAidTopic, type FirstAidTopicSection } from "../lib/firstAidTopicsApi";
import MediaPicker from "./MediaPicker";

const EMPTY: FirstAidTopic = {
  id: "",
  order_num: 0,
  title_ar: "",
  title_en: "",
...
export default function FirstAidAdmin() {
  const [items, setItems] = useState<FirstAidTopic[]>([]);
  const [editing, setEditing] = useState<FirstAidTopic | null>(null);
  const [sections, setSections] = useState<FirstAidTopicSection[]>([]);
  ...
  function load() {
    setLoading(true);
    fetchFirstAidTopics().then(setItems).catch(() => setItems([])).finally(() => setLoading(false));
  }

  useEffect(() => { load(); }, []);

  function startEdit(p: FirstAidTopic | null) {
  ...
  function updateSection(idx: number, patch: Partial<FirstAidTopicSection>) {
  ...
  return (
    <div dir="rtl" className="mx-auto max-w-4xl p-6">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-xl font-bold dark:text-white">🩹 إدارة قسم الإسعافات الأولية (First Aid) ({items.length})</h1>
