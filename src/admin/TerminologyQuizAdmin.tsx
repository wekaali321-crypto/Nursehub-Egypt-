...
import { fetchTerminologyQuizQuestions, upsertTerminologyQuizQuestion, deleteTerminologyQuizQuestion, type TerminologyQuizQuestion } from "../lib/terminologyQuizApi";
...
const blank = (nextOrder: number): TerminologyQuizQuestion => ({
  id: "tq" + Date.now(),
  order_num: nextOrder,
  category: "",
  text_ar: "",
...
export default function TerminologyQuizAdmin() {
  const { notify } = useToast();
  const [items, setItems] = useState<TerminologyQuizQuestion[] | null>(null);
  const [editing, setEditing] = useState<TerminologyQuizQuestion | null>(null);

  const reload = () => fetchTerminologyQuizQuestions().then(setItems).catch(() => setItems([]));
  useEffect(() => { reload(); }, []);
  ...
  const save = async () => {
    ...
    try {
      await upsertTerminologyQuizQuestion(editing);
      notify("تم الحفظ");
      setEditing(null);
      reload();
    } catch (e) {
  ...
  const del = async (id: string) => {
    ...
    await deleteTerminologyQuizQuestion(id);
    notify("تم الحذف");
    reload();
  };
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={save} className="rounded-lg bg-violet-500 px-6 py-2 text-sm font-bold text-white">حفظ</button>
          </div>
        </div>

        <div className={`grid gap-3 sm:grid-cols-2 ${card}`}>
          <input placeholder="التصنيف (مثال: أساسيات بناء الكلمة)" value={editing.category || ""} onChange={(e) => setEditing({ ...editing, category: e.target.value })} className={inp} />
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-slate-500 dark:text-slate-400">عدد الأسئلة: <span className="font-bold text-violet-500">{items?.length ?? "…"}</span></p>
        <button onClick={() => setEditing(blank((items?.length || 0) + 1))} className="rounded-lg bg-gradient-to-l from-violet-600 to-fuchsia-500 px-5 py-2 text-sm font-bold text-white">+ سؤال جديد</button>
      </div>
          ...
          items.map((q) => (
            <div key={q.id} className={`flex items-center justify-between ${card} py-3`}>
              ...
              <div className="flex shrink-0 gap-1">
                <button onClick={() => setEditing(q)} className="rounded-lg bg-violet-100 px-3 py-1 text-xs font-bold text-violet-600 dark:bg-violet-500/10">تعديل</button>
