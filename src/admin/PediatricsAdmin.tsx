...
import { fetchPediatricTopics, upsertPediatricTopic, deletePediatricTopic, type PediatricTopic, type PediatricTopicSection } from "../lib/pediatricTopicsApi";
import MediaPicker from "./MediaPicker";

const EMPTY: PediatricTopic = {
  id: "",
  order_num: 0,
  title_ar: "",
  title_en: "",
...
export default function PediatricsAdmin() {
  const [items, setItems] = useState<PediatricTopic[]>([]);
  const [editing, setEditing] = useState<PediatricTopic | null>(null);
  const [sections, setSections] = useState<PediatricTopicSection[]>([]);
  ...
  function load() {
    setLoading(true);
    fetchPediatricTopics().then(setItems).catch(() => setItems([])).finally(() => setLoading(false));
  }

  useEffect(() => { load(); }, []);

  function startEdit(p: PediatricTopic | null) {
  ...
  function updateSection(idx: number, patch: Partial<PediatricTopicSection>) {
  ...
  return (
    <div dir="rtl" className="mx-auto max-w-4xl p-6">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-xl font-bold dark:text-white">🍼 إدارة قسم الأطفال (Pediatrics) ({items.length})</h1>
        <button onClick={() => startEdit(null)} className="rounded-lg bg-sky-600 px-4 py-2 text-white">+ إضافة موضوع</button>
      </div>
          ...
          {items.map((p) => (
            <div key={p.id} className="flex items-center justify-between rounded-xl border border-slate-100 bg-white p-3 dark:border-slate-800 dark:bg-slate-900">
              ...
              <div className="flex gap-2">
                <button onClick={() => startEdit(p)} className="text-sm text-sky-700 dark:text-sky-400">تعديل</button>
      ...
      {editing && (
        <div className="space-y-4 rounded-xl border border-slate-100 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
          ...
          <div className="flex gap-3 border-t border-slate-200 pt-2 dark:border-slate-800">
            <button onClick={save} className="rounded-lg bg-sky-600 px-5 py-2 text-white">حفظ</button>
