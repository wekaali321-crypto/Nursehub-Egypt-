...
function CalcBlock({ dose }: { dose: Record<string, string> | null }) {
  ...
  return (
    <div className="mb-4 rounded-xl bg-teal-50 border border-teal-100 p-4 dark:bg-teal-500/10 dark:border-teal-500/20">
      <h3 className="font-bold text-teal-700 dark:text-teal-400 mb-2">جدول الجرعات</h3>
...
export default function PedsMedicationDetail() {
  ...
  if (loading) return <div className="p-8 text-center text-slate-500 dark:text-slate-400">جارِ التحميل...</div>;
  if (!item) return <div className="p-8 text-center text-slate-500 dark:text-slate-400">لم يتم العثور على الحالة.</div>;
