...
export default function DrugAntidotesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return drugAntidotes.filter(
      (a) => a.toxin.toLowerCase().includes(s) || a.antidotes.toLowerCase().includes(s)
    );
  }, [drugAntidotes, q]);

  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      <Breadcrumbs items={[{ label: "الأدوية", path: "/drugs" }, { label: "الترياقات" }]} />

      <div className="mb-6 rounded-3xl bg-gradient-to-l from-emerald-600 to-teal-500 p-6 text-white sm:p-8">
        <div className="text-4xl sm:text-5xl">🧪</div>
        <h1 className="mt-2 text-2xl font-black sm:text-3xl">الترياقات الطبية (Antidotes)</h1>
        <p className="mt-1 text-emerald-50">{drugAntidotes.length} حالة تسمم مع الترياق المناسب لها — مرجع سريع للطوارئ.</p>
      </div>
      ...
      <input
        ...
        placeholder="ابحثي عن مادة سامة أو ترياق..."
        ...
        {filtered.map((a) => (
          <div key={a.id} className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
            <div className="flex flex-wrap items-start justify-between gap-2">
              <h3 className="font-bold text-slate-900 dark:text-white">{a.toxin}</h3>
              <span className="rounded-full bg-emerald-50 px-3 py-1 text-sm font-bold text-emerald-600 dark:bg-emerald-500/10">💊 {a.antidotes}</span>
            </div>
