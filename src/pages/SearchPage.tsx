...
export default function SearchPage() {
  ...
  const { icuTopics, dialysisTopics, nicuTopics, pharmacologyTopics, terminologyTopics, firstAidTopics, erReceptionTopics, pediatricTopics, anatomyTopics, fundamentalsTopics, operatingRoomTopics, obgynTopics, protocols } = useExtraSearchIndex();
  ...
  const erReceptionResults = q ? erReceptionTopics.filter((topic) => [topic.title_ar, topic.title_en, topic.summary_ar, topic.summary_en].filter(Boolean).join(" ").toLowerCase().includes(lower)) : [];
  const pediatricsResults = q ? pediatricTopics.filter((topic) => [topic.title_ar, topic.title_en, topic.summary_ar, topic.summary_en].filter(Boolean).join(" ").toLowerCase().includes(lower)) : [];
  const anatomyResults = q ? anatomyTopics.filter((topic) => [topic.title_ar, topic.title_en, topic.summary_ar, topic.summary_en].filter(Boolean).join(" ").toLowerCase().includes(lower)) : [];
  const fundamentalsResults = q ? fundamentalsTopics.filter((topic) => [topic.title_ar, topic.title_en, topic.summary_ar, topic.summary_en].filter(Boolean).join(" ").toLowerCase().includes(lower)) : [];
  const operatingRoomResults = q ? operatingRoomTopics.filter((topic) => [topic.title_ar, topic.title_en, topic.summary_ar, topic.summary_en].filter(Boolean).join(" ").toLowerCase().includes(lower)) : [];
  const obgynResults = q ? obgynTopics.filter((topic) => [topic.title_ar, topic.title_en, topic.summary_ar, topic.summary_en].filter(Boolean).join(" ").toLowerCase().includes(lower)) : [];
    ...
    <div className="mx-auto max-w-7xl px-4 py-8">
      ...
      {total === 0 ? (
        ...
        <div className="space-y-8">
          ...
          {showContent && dialysisResults.length > 0 && (
            <div>
              <h2 className="mb-3 text-lg font-bold dark:text-white">{t("search.dialysisHeading")} ({dialysisResults.length})</h2>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {dialysisResults.map((topic) => {
                  const title = bilingual(topic.title_ar, topic.title_en, lang).text;
                  const summary = bilingual(topic.summary_ar, topic.summary_en, lang).text;
                  return (
                    <Link key={topic.id} to={`/dialysis/${topic.id}`} className="rounded-xl border border-slate-200 bg-white p-4 hover:border-sky-400 dark:border-slate-800 dark:bg-slate-900">
                      <div className="font-bold dark:text-white">{topic.icon || "💧"} {title}</div>
                      {summary && <div className="mt-1 line-clamp-2 text-sm text-slate-400">{summary}</div>}
                    </Link>
                  );
          ...
          {showContent && nicuResults.length > 0 && (
            <div>
              <h2 className="mb-3 text-lg font-bold dark:text-white">{t("search.nicuHeading")} ({nicuResults.length})</h2>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {nicuResults.map((topic) => {
                  const title = bilingual(topic.title_ar, topic.title_en, lang).text;
                  const summary = bilingual(topic.summary_ar, topic.summary_en, lang).text;
                  return (
          ...
          {showContent && terminologyResults.length > 0 && (
            <div>
              <h2 className="mb-3 text-lg font-bold dark:text-white">{t("search.terminologyHeading")} ({terminologyResults.length})</h2>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {terminologyResults.map((topic) => {
                  const title = bilingual(topic.title_ar, topic.title_en, lang).text;
                  const summary = bilingual(topic.summary_ar, topic.summary_en, lang).text;
                  return (
                    <Link key={topic.id} to={`/terminology/${topic.id}`} className="rounded-xl border border-slate-200 bg-white p-4 hover:border-sky-400 dark:border-slate-800 dark:bg-slate-900">
                      <div className="font-bold dark:text-white">{topic.icon || "🔤"} {title}</div>
                      {summary && <div className="mt-1 line-clamp-2 text-sm text-slate-400">{summary}</div>}
                    </Link>
                  );
          ...
          {showContent && firstAidResults.length > 0 && (
            <div>
              <h2 className="mb-3 text-lg font-bold dark:text-white">{t("search.firstAidHeading")} ({firstAidResults.length})</h2>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {firstAidResults.map((topic) => {
                  const title = bilingual(topic.title_ar, topic.title_en, lang).text;
                  const summary = bilingual(topic.summary_ar, topic.summary_en, lang).text;
                  return (
                    <Link key={topic.id} to={`/first-aid/${topic.id}`} className="rounded-xl border border-slate-200 bg-white p-4 hover:border-sky-400 dark:border-slate-800 dark:bg-slate-900">
                      <div className="font-bold dark:text-white">{topic.icon || "🩹"} {title}</div>
                      {summary && <div className="mt-1 line-clamp-2 text-sm text-slate-400">{summary}</div>}
                    </Link>
                  );
          ...
          {showContent && erReceptionResults.length > 0 && (
            <div>
              <h2 className="mb-3 text-lg font-bold dark:text-white">{t("search.erReceptionHeading")} ({erReceptionResults.length})</h2>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {erReceptionResults.map((topic) => {
                  const title = bilingual(topic.title_ar, topic.title_en, lang).text;
                  const summary = bilingual(topic.summary_ar, topic.summary_en, lang).text;
                  return (
                    <Link key={topic.id} to={`/er-reception/${topic.id}`} className="rounded-xl border border-slate-200 bg-white p-4 hover:border-sky-400 dark:border-slate-800 dark:bg-slate-900">
                      <div className="font-bold dark:text-white">{topic.icon || "🚨"} {title}</div>
                      {summary && <div className="mt-1 line-clamp-2 text-sm text-slate-400">{summary}</div>}
                    </Link>
                  );
          ...
          {showContent && pediatricsResults.length > 0 && (
            <div>
              <h2 className="mb-3 text-lg font-bold dark:text-white">{t("search.pediatricsHeading")} ({pediatricsResults.length})</h2>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {pediatricsResults.map((topic) => {
                  const title = bilingual(topic.title_ar, topic.title_en, lang).text;
                  const summary = bilingual(topic.summary_ar, topic.summary_en, lang).text;
                  return (
                    <Link key={topic.id} to={`/pediatrics/${topic.id}`} className="rounded-xl border border-slate-200 bg-white p-4 hover:border-sky-400 dark:border-slate-800 dark:bg-slate-900">
                      <div className="font-bold dark:text-white">{topic.icon || "🍼"} {title}</div>
                      {summary && <div className="mt-1 line-clamp-2 text-sm text-slate-400">{summary}</div>}
                    </Link>
                  );
          ...
          {showContent && anatomyResults.length > 0 && (
            <div>
              <h2 className="mb-3 text-lg font-bold dark:text-white">{t("search.anatomyHeading")} ({anatomyResults.length})</h2>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {anatomyResults.map((topic) => {
                  const title = bilingual(topic.title_ar, topic.title_en, lang).text;
                  const summary = bilingual(topic.summary_ar, topic.summary_en, lang).text;
                  return (
                    <Link key={topic.id} to={`/anatomy/${topic.id}`} className="rounded-xl border border-slate-200 bg-white p-4 hover:border-amber-400 dark:border-slate-800 dark:bg-slate-900">
                      <div className="font-bold dark:text-white">{topic.icon || "🦴"} {title}</div>
                      {summary && <div className="mt-1 line-clamp-2 text-sm text-slate-400">{summary}</div>}
                    </Link>
                  );
          ...
          {showContent && fundamentalsResults.length > 0 && (
            <div>
              <h2 className="mb-3 text-lg font-bold dark:text-white">{t("search.fundamentalsHeading")} ({fundamentalsResults.length})</h2>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {fundamentalsResults.map((topic) => {
                  const title = bilingual(topic.title_ar, topic.title_en, lang).text;
                  const summary = bilingual(topic.summary_ar, topic.summary_en, lang).text;
                  return (
                    <Link key={topic.id} to={`/fundamentals/${topic.id}`} className="rounded-xl border border-slate-200 bg-white p-4 hover:border-blue-400 dark:border-slate-800 dark:bg-slate-900">
                      <div className="font-bold dark:text-white">{topic.icon || "🛏️"} {title}</div>
                      {summary && <div className="mt-1 line-clamp-2 text-sm text-slate-400">{summary}</div>}
                    </Link>
                  );
                })}
          ...
          {showContent && operatingRoomResults.length > 0 && (
            <div>
              <h2 className="mb-3 text-lg font-bold dark:text-white">{t("search.operatingRoomHeading")} ({operatingRoomResults.length})</h2>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {operatingRoomResults.map((topic) => {
                  const title = bilingual(topic.title_ar, topic.title_en, lang).text;
                  const summary = bilingual(topic.summary_ar, topic.summary_en, lang).text;
                  return (
                    <Link key={topic.id} to={`/operating-room/${topic.id}`} className="rounded-xl border border-slate-200 bg-white p-4 hover:border-emerald-400 dark:border-slate-800 dark:bg-slate-900">
                      <div className="font-bold dark:text-white">{topic.icon || "🔪"} {title}</div>
                      {summary && <div className="mt-1 line-clamp-2 text-sm text-slate-400">{summary}</div>}
                    </Link>
                  );
          ...
          {showContent && obgynResults.length > 0 && (
            <div>
              <h2 className="mb-3 text-lg font-bold dark:text-white">{t("search.obgynHeading")} ({obgynResults.length})</h2>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {obgynResults.map((topic) => {
                  const title = bilingual(topic.title_ar, topic.title_en, lang).text;
                  const summary = bilingual(topic.summary_ar, topic.summary_en, lang).text;
                  return (
                    <Link key={topic.id} to={`/obgyn/${topic.id}`} className="rounded-xl border border-slate-200 bg-white p-4 hover:border-rose-400 dark:border-slate-800 dark:bg-slate-900">
                      <div className="font-bold dark:text-white">{topic.icon || "🤰"} {title}</div>
                      {summary && <div className="mt-1 line-clamp-2 text-sm text-slate-400">{summary}</div>}
                    </Link>
                  );
