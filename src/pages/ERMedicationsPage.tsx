...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  "الأدوية المخدرة (Narcotics)": "from-indigo-500 to-violet-600",
  "مضادات القيء والحموضة (Antiemetics & GI)": "from-lime-500 to-green-600",
  "الحساسية والكورتيزون ومضادات الهيستامين (Allergy & Corticosteroids)": "from-pink-500 to-rose-600",
  "موسعات الشعب الهوائية (Bronchodilators)": "from-sky-500 to-cyan-600",
  "أدوية القلب والأوعية الدموية (Cardiac & Vasoactive Drugs)": "from-red-600 to-rose-700",
  "الطوارئ السكرية (Diabetic Emergencies)": "from-purple-500 to-fuchsia-600",
  "المهدئات ومضادات الاختلاج (Sedatives & Anticonvulsants)": "from-slate-500 to-slate-700",
  "فيتامينات ووقاية (Vitamins & Prophylaxis)": "from-teal-500 to-emerald-600",
};
...
const LABELS = {
  ...
  back: { ar: "→ العودة لأدوية الطوارئ", en: "→ Back to ER medications" },
  ...
  subtitle: { ar: "دليل شامل لأدوية الطوارئ الشائعة: الاستعمالات، موانع الاستخدام، طريقة التحضير، وحساب الجرعات.", en: "A comprehensive guide to common ER medications: uses, contraindications, preparation, and dose calculation." },
...
function CalcBlock({ dose, title }: { dose: Record<string, string> | null; title: string }) {
  if (!dose) return null;
  return (
    <div className="mb-4 rounded-xl bg-red-50 border border-red-100 p-4 dark:bg-red-500/10 dark:border-red-500/20">
...
export function ERMedicationsHome() {
  const { lang } = useI18n();
  ...
  const grouped = ER_CATEGORY_ORDER.map((cat) => ({
    category: cat,
    ...
    <div dir={lang === "ar" ? "rtl" : "ltr"} className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="relative mb-8">
        <input
          ...
          className="w-full rounded-lg border border-slate-200 bg-white py-2.5 pr-10 pl-3 outline-none focus:border-red-400 dark:border-slate-700 dark:bg-slate-800 dark:text-white"
        />
        ...
        <div key={g.category} className="mb-8">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {g.drugs.map((d) => (
              <Link
                key={d.id}
                to={`/drugs/er-medications/${d.id}`}
                className={`rounded-xl p-4 text-white bg-gradient-to-l ${CATEGORY_COLORS[g.category] || "from-red-500 to-rose-700"} hover:opacity-90 transition`}
              >
...
export default function ERMedicationDetail() {
  const { id } = useParams();
  const { lang } = useI18n();
  const [item, setItem] = useState<ERMedication | null>(null);
  const [loading, setLoading] = useState(true);
  ...
  return (
    <div dir={lang === "ar" ? "rtl" : "ltr"} className="max-w-3xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-4 print:hidden">
        <Link to="/drugs/er-medications" className="text-red-600 text-sm inline-block">{LABELS.back[lang]}</Link>
      ...
      <div className="print:hidden"><SafetyLinksBox table="er_medications" id={item.id} /></div>
