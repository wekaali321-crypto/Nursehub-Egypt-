...
const fromDrug = (r: any) => ({ id: r.id, name: r.name, genericName: r.generic_name, drugClass: r.drug_class, category: r.category, dose: r.dose, indications: r.indications, sideEffects: r.side_effects, nursingConsiderations: r.nursing_considerations, contraindications: r.contraindications, storage: r.storage, references: r.references, slug: r.slug, isHighAlert: r.is_high_alert ?? false, highAlertWarnings: r.high_alert_warnings ?? "", imageUrl: r.image_url ?? "", showImage: r.show_image ?? false });
const toDrug = (d: any) => ({ id: d.id, name: d.name, generic_name: d.genericName, drug_class: d.drugClass, category: d.category, dose: d.dose, indications: d.indications, side_effects: d.sideEffects, nursing_considerations: d.nursingConsiderations, contraindications: d.contraindications ?? "", storage: d.storage ?? "", references: d.references ?? "", slug: d.slug, is_high_alert: d.isHighAlert ?? false, high_alert_warnings: d.highAlertWarnings ?? "", image_url: d.imageUrl ?? "", show_image: d.showImage ?? false });
