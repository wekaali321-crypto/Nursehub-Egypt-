...
const fromDrug = (r: any) => ({ id: r.id, name: r.name, genericName: r.generic_name, drugClass: r.drug_class, category: r.category, dose: r.dose, indications: r.indications, sideEffects: r.side_effects, nursingConsiderations: r.nursing_considerations, contraindications: r.contraindications, storage: r.storage, references: r.references, slug: r.slug, isHighAlert: r.is_high_alert ?? false, highAlertWarnings: r.high_alert_warnings ?? "", highAlertType: r.high_alert_type ?? [], imageUrl: r.image_url ?? "", showImage: r.show_image ?? false });
